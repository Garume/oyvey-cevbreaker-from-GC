...
import static me.alpha432.oyvey.util.SpoofRotationUtil.*;

import java.util.ArrayList;
import java.util.Objects;

import  me.alpha432.oyvey.event.Phase;
import me.alpha432.oyvey.event.events.DestroyBlockEvent;
import me.alpha432.oyvey.event.events.OnUpdateWalkingPlayerEvent;
import me.alpha432.oyvey.event.events.PacketEvent;
import me.alpha432.oyvey.features.modules.Module;
import me.alpha432.oyvey.features.setting.Setting;
import me.alpha432.oyvey.manager.PlayerPacketManager;
import  me.alpha432.oyvey.util.BlockUtil;
import  me.alpha432.oyvey.util.CrystalUtil;
import  me.alpha432.oyvey.util.EntityUtil;
import  me.alpha432.oyvey.util.HoleUtil;
import me.alpha432.oyvey.util.PlacementUtil;
import me.alpha432.oyvey.util.PlayerPacket;
...
public class CevBreaker extends Module {
    ...
    public Setting<TargetMode> target = this.register(new Setting<TargetMode>("Target", TargetMode.Nearest));
    public Setting<CbreakMode> breakCrystal = this.register(new Setting<CbreakMode>("breakCrystal", CbreakMode.Packet));
    public Setting<BbreakMode> breakBlock = this.register(new Setting<BbreakMode>("breakBlock", BbreakMode.Packet));

    private final Setting<Float> enemyRange = this.register(new Setting("Range", Float.valueOf(4.9F), Float.valueOf(0.0F), Float.valueOf(6.0F)));
    private final Setting<Integer>preRotationDelay = this.register(new Setting<Integer>("preRotationDelay", 0, 0, 20));
    private final Setting<Integer>afterRotationDelay = this.register(new Setting<Integer>("After Rotation Delay", 0, 0, 20));
    private final Setting<Integer> supDelay = this.register(new Setting<Integer>("Support Delay", 1, 0, 4));
    private final Setting<Integer> crystalDelay = this.register(new Setting<Integer>("Crystal Delay", 2, 0, 20));
    private final Setting<Integer> blocksPerTick = this.register(new Setting<Integer>("Blocks Per Tick", 4, 2, 6));
    private final Setting<Integer> hitDelay = this.register(new Setting<Integer>("Hit Delay", 2, 0, 20));
    private final Setting<Integer> midHitDelay = this.register(new Setting<Integer>("Mid Hit Delay", 1, 0, 20));
    private final Setting<Integer> endDelay = this.register(new Setting<Integer>("End Delay", 1, 0, 20));
    private final Setting<Integer> pickSwitchTick = this.register(new Setting<Integer>("Pick Switch Tick", 100, 0, 500));

    private  Setting<Boolean> rotate = this.register(new Setting<Boolean>("Rotate", false));
    private  Setting<Boolean> confirmBreak = this.register(new Setting<Boolean>("No Glitch Break", true));
    private  Setting<Boolean> confirmPlace = this.register(new Setting<Boolean>("No Glitch Place", true));
    private  Setting<Boolean> antiWeakness = this.register(new Setting<Boolean>("Anti Weakness", false));
    private  Setting<Boolean> switchSword = this.register(new Setting<Boolean>("Switch Sword", false));
    private  Setting<Boolean> fastPlace = this.register(new Setting<Boolean>("Fast Place", false));
    private  Setting<Boolean> fastBreak = this.register(new Setting<Boolean>("Fast Break", true));
    private  Setting<Boolean> trapPlayer = this.register(new Setting<Boolean>("Trap Player", false));
    private  Setting<Boolean> antiStep = this.register(new Setting<Boolean>("Anti Step", false));
    private  Setting<Boolean> placeCrystal = this.register(new Setting<Boolean>("Place Crystal", true));
    private  Setting<Boolean> forceRotation = this.register(new Setting<Boolean>("Force Rotation", false));
    private  Setting<Boolean> forceBreaker = this.register(new Setting<Boolean>("Force Breaker", false));


    public CevBreaker() {
    	super( "CevBreaker","cevbreaker", Module.Category.COMBAT, true, false, false);
    }
                ...
                case 3:
                   ...
                   switchPick(switchValue);
                    ...
                    if (BlockUtil.getBlock(obbyBreak) instanceof BlockObsidian) {
                        ...
                        if (sideBreak != null) {
                            // Switch break values
                        	if (this.breakBlock.getValue() == BbreakMode.Packet) {
                                // Normal Packet
                                    if (!prevBreak) {
                                ...
                                // Vanilla
                        	else if  (this.breakBlock.getValue() == BbreakMode.Normal) {
                                    mc.player.swingArm(EnumHand.MAIN_HAND);
    ...
    private void breakCrystalPiston(Entity crystal) {
        ...
        if (this.breakCrystal.getValue() != CbreakMode.None) {
            if (rotate.getValue()) {
                // Look at that packet
        ...
        try {
        	if (this.breakCrystal.getValue() == CbreakMode.Vanilla) {
                    CrystalUtil.breakCrystal(crystal);
        	}
        	else if (this.breakCrystal.getValue() == CbreakMode.Packet) {
                        CrystalUtil.breakCrystalPacket(crystal);
        	}else if (this.breakCrystal.getValue() == CbreakMode.None) {}
    ...
    public enum CbreakMode {
        Vanilla,
        Packet,
        None

    }
    public enum BbreakMode {
        Normal,
        Packet
